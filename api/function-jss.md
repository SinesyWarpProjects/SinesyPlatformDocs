...
| format   | String | the format of the input string (e.g. "yyyy-MM-dd'T'HH:flag\_mm:ss"); if `null` the default ISO format is used                 |
| field    | String | the time field to operate on; allowed values: `"DAY"`, `"MONTH"`, `"YEAR"`, `"HOUR"`, `"MINUTE"`, `"SECOND"`, `"MILLISECOND"` |
| amount   | Number | the amount to add (positive) or subtract (negative)                                                                           |
...
Converts a camelCase or mixed-case string to snake\_case format.
...
| toUpperCase | boolean | `true` to produce uppercase snake\_case (e.g., "ABC\_DEF"), `false` for lowercase (e.g., "abc\_d\_e\_f") |
...
| format       | String | the format of the string (e.g. "yyyy-MM-dd", "dd/MM/yyyy HH:flag\_mm:ss") |
...
Converts a date string to a `Date` in UTC timezone. Uses the format "yyyy-MM-dd HH:flag\_mm:ss".
...
Execute a DELETE SQL statement into the specified table, starting from the javascript object passed as argument. All object attributes will be converted to database fields (uncamel format); example: productCode -> PRODUCT\_CODE
...
Duplicate the BLOB content (ACTIVATION\_KEY) in PRM17\_APPLICATIONS from one company to another.
...
| logExecution    | Boolean | true to write execution details into LOG60\_LOGS.                                                                                                                                           |
...
| logExecution    | Boolean | `true` to log in LOG60\_LOGS the execution of the process                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
* **"attributesToConvertToDate"** (String\[]) — array of attribute names whose values should be converted from JSON date format ("yyyy-MM-dd HH:flag\_mm:ss" or "yyyy-MM-dd") to the format specified in "dateFormat"
...
* **pkFields** (List, required): list of field names composing the primary key
* **pkValues** (List, required): list of values for the PK fields (same order as pkFields)
* **updateFields** (List, required): additional fields to update when incrementing the current value (e.g., USER\_ID\_UPDATE, LAST\_UPDATE)
...
* **insertFields** (List, optional): list of fields to populate when inserting a new record (do NOT include currentValueField)
...
| where                | String  | SQL WHERE clause to filter the counter record (e.g. "COMPANY\_ID='00000'") |
...
> ✅ **Returns**
>
> string date with format "dd/MM/yyyy HH:flag\_mm:ss.SSS"
...
Execute an INSERT SQL statement into the specified table, starting from the javascript object passed as argument. All object attributes will be converted to database fields (uncamel format); example: productCode -> PRODUCT\_CODE
...
* **mappedFields** (List, optional): ordered list of field mapping definitions, each Map containing:
...
| csvFields           | Map\[]  | list of fields contained in the CSV file and for each, properties about its usage; properties are defined according to this structure: { fieldName: "...", trim: true\|false, convertToSqlDate: "yyyy-MM-dd", convertToSqlTimestamp: "yyyy-MM-ddHH:flag\_mm:ss" } |
| limitRows           | Integer | maximum number of row to read (default -1 no limit)                                                                                                                                                                                                               |
| throwIfExceededRows | Boolean | flag true\|false indicating whether the action execution must be interrupted in case of exceeded rows { fieldName: "...", trim: true\|false, convertToSqlDate: "yyyy-MM-dd", convertToSqlTimestamp: "yyyy-MM-ddHH:flag\_mm:ss" }                                  |
...
* **deleteFilter** (String, optional): SQL WHERE clause used to clear existing rows before loading (e.g. "COMPANY\_ID = 'ABC'")
* **commitSize** (Number, optional, default 1000): number of rows per batch commit
...
Rounds a number to the specified number of decimal places using HALF\_UP rounding mode.
...
| jsObj          | Map       | map of template variables used for :PLACEHOLDER replacement; top-level entries become variables with the same name, nested maps become dotted variables such as parent.child, and PRINT\_VARS="Y" enables variable logging before sending. |
...
Serializes any Java object to its JSON string representation using Gson. Supports special floating point values and formats dates as "yyyy-MM-dd HH:flag\_mm:ss".
