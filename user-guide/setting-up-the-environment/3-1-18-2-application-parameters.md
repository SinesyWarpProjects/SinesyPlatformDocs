...
{% content-ref url="3-1-18-2-application-parameters.md" %}
[3-1-18-2-application-parameters.md](3-1-18-2-application-parameters.md)
{% endcontent-ref %}
...
**Alert message template after import** - optional text, used when the end user is importing asynchronously a csv/xls file from a grid and it has been defined the "Directory Id where saving report when importing data" and the max number of errors > 1&#x20;

Example:

```
<div class=\"notifica\" style=\"width: 450px; float:left; cursor:pointer;\"> 
                <div style=\"width: 350px;float:left;\"> 
           <img src='../images/elaborated.png' style=\"float:left; width:30px; margin:5px 20px 0 0; vertical-align:middle;\">
            <div style=\"float: right;width: 300px;\"> 
                        <p style=\"width:100%; padding:3px 0 3px 0;\"><b>{title}</b></p> 
                        <p style=\"width:100%; padding:3px 0 3px 0;color: grey;white-space:normal;\"><a href=# onClick='window.open(&#39;{reportUrl}&deleteFile=Y&#39;,&#39;Report&#39;); markMessageAsRead(&#39;ALERT&#39;,{logId});'  >{filename}</a></p> 
                    </div> 
                </div> 
            </div>
```

The template can contain a few variables, surrounded by {}.

Supported variables:

* **reportUrl** - the URL of the xls report file, generated during the import and containing the errors
* **filename** - the imported csv/xls file
* **title** - title to show in the notification message; it will be the translation of the following entry: “configureimpfile.importfileterminated“
* **errorMessage** - the error message, in case of errors generated during the import
* **logId** - record id for the alert message stored in CON60\_LOGS
* **countErrors** - total number of errors

**Alert message template with errors in import** - optional text, used when the end user is importing asynchronously a csv/xls file from a grid and it has been defined the "Directory Id where saving report when importing data" and the max number of errors > 1  and there are fatal errors
...
<div class=\"notifica\" style=\"width: 450px; float:left; cursor:pointer;\"> 
                <div style=\"width: 350px;float:left;\"> 
           <img src='../images/error.png' style=\"float:left; width:30px; margin:5px 20px 0 0; vertical-align:middle;\">
            <div style=\"float: right;width: 300px;\"> 
                        <p style=\"width:100%; padding:3px 0 3px 0;\"><b>{title}</b></p> 
                        <p style=\"width:100%; padding:3px 0 3px 0;color: grey;white-space:normal;\"><a href=# onClick='window.open(&#39;{reportUrl}&deleteFile=Y&#39;,&#39;Report&#39;); markMessageAsRead(&#39;ALERT&#39;,{logId});'  >{filename}</a></p> 
                    </div> 
...
Directory Id where saving report when importing data - this can be a bucket in GCS or a local directory; it is required in order to generate a report in xls for errors during the import
...
exit,companyId=00000,siteId=100,forgotpwd=false
...
If a user is not defined in Platform too, it would be impossible to refer it and link to it a set of roles!<br>

To enable this feature, the following parameters must be set in the general configuration or the application specific configuration.<br>
...
**Custom CSS** - optional property where setting CSS code to include in the login page and in the main page of the interpreter application (after login).
...
Thanks to that, in a multi-tenancy environment having a cluster, tenant's logo can be save in Google Cloud Storage once and referred dynamically here, with something like:
