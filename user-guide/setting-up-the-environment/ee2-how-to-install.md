...
* **Java 17 new version** - this is the most recent version of **Platform: 7.0 or greater;** this 7.0 version support the new UI based on ReactJs and PrimeReact
* **Java 11  version (deprecated)** - this is **Platform: 6.x;** the 6.x version is functionally equivalent to 5.2.3 version
* **Java 1.7 based version (not anymore supported)** - Platform started with this version and it spreads up to the **5.2.3 version**
...
## INSTALLING 4WS.PLATFORM COMMUNITY EDITION - JAVA 17

* **Install OpenJDK 17.x**  in your machine, if not already available; please, **do NOT use other versions**, such as Oracle JDK 17, otherwise some parts of the product would not work correctly and it would represent a license violation of Oracle JVM. You can find OpenJSK distribution at these links, for each  operating system:
...
| **MySQL / Google CloudSQL** - release 8.x or fewer    |
| **PostgreSQL**                                        |
...
* 4WS.Platform requires a Java web container compatible with Java 17: Platform installer already includes Tomcat 10.1.x. If you have already installed it for other purposes, you can reuse it; if you do not have installed it yet, the 4WS.Platform installer will install it along with the product, since it is included in the distribution.&#x20;
* **Decompress** the .zip file downloaded from the repository and **execute the installer** .\
  **If you are using recent versions of Windows, you have to use a superuser and open a DOS prompt by right clicking on it and choose “Run as Administrator”: that is the right way to install the program** . **DO NOT simply execute the installer using a superuser (e.g. administrator), since this has not the same effect** .\
...
* **installation path**, where Tomcat will be installed; if you want to reuse an already existing Tomcat 10.1.x installation, you can simply specify that path and the installer will skip the Tomcat installation task and will install and configure the 4WS.Platform web application only
...
**It is recommended to use Chrome or Mozilla Firefox browsers** .
...
## PORTING FROM JAVA 1.7 TO JAVA 11 or JAVA 17
