...
* bulk import from the server file system – in that case, documents should have been already moved to the local file system, so **it is unlikely to follow this approach, more feasible for “on premise” installations of 4Ws.Platform**
...
**The CSV file must respect the standard CSV format**: files not respecting such a standard will generate errors.\
...
* one of the columns must be related to the files to import: every row in the CSV must refer a file in the remote repository containing documents and csv
* csv file and documents must be contained in the same area
...
The csv file could contain less data than the ones required as metadata for a document: additional metadata not provided by the csv could be defined:
...
In case of complex logic to carry out for each row to import, you can rely on two optional server-side js actions: one is invoked just before the processing of each row in the csv file; the second is invoked just after importing metadata and document within the CMS.\
...
The alternative to a bulk import from FTP is the local bulk import, where all documents and CSV file are temporarelly stored in a path accessible from the server. In such a case, the js function to use is:

```javascript
var files = utils.bulkImport(
      Long sourceDirId,
      Long destDirId,
      Long backupDirId,
      Long dataSourceId,
      String tableName,
      String csvFileName,
      String csvFileNameField
      ,String csvUniqueIdField,
      String csvSep,
      Map<String,Object> defaultValues,
      Map<String,String> mapping,
      String nullString,
      Boolean skipWithErrors,
      Boolean skipFileNotInCSV,
      Long beforeInsertActionId,
      Long afterInsertActionId)
```

Import all files found in the location identified by sourceDirId and load them into the location identified by destDirId. If backupDirId is not null, input files are also moved (removed from the source folder) to the location identified by backupDirId.
...
Meaning of the function arguments:
...
* csvSep ; or , symbol
* defaultValues collection of attribute names + default values, to set in case the metadata does not provide a value for these attributes; :XXX variables are supporte
...
* skipWithErrors \<code>true\</code> to undo any insert in case of errors
* skipFileNotInCSV \<code>true\</code> to skip file loading if the same file was not found in the CSV file
...
It returns a list of files skipped because of some error.



## **Creating a CSV file from the local folder structure**

In case original files to import are temporarelly stored in the local server filer system (or any  shared folder), you can automate the creation of a CSV file, starting from the base dir where these files are stored, using the following js function:
...
* dirId - base dir where searching for files
* destDirId - base dir where saving the CSV foile
* csvFileName - CSV file name to create
* csvSeparator - character to use to separate fields (e.g. "," or ";")
* fileNameAttributes - the parts composing the file name, separated with each other by "fileNameSeparator"
* fileNameSeparator - the character to use for separing attributes in the file name (e.g. "\_")
* folderAttributes - list of attributes identifying the folders containing a file
...
utils.createCsvFromFolders(
    ...
    "cms.csv",
    ";",
    "Mese_Giorno_NumDoc", // fileNameAttributes,
    "_", // fileNameSeparator
    ["TipoDoc","Anno","CliForn"] // folderAttributes
);

```

related to a file system structure like:

* ODA
  * 2024
    * Google
      * 01\_31\_001\_myfile.pdf
      * 02\_02\_002\_myfile.pdf
    * Amazon
      * 01\_31\_003\_myfile.pdf
* ODV
  * 2023
    * Client1
      * 04\_30\_004\_myfile.pdf
    * Client2
      * 05\_02\_005\_myfile.pdf





## **Creating a CSV file from a folder structure stored in an FTP server**

In case original files to import are temporarelly stored in an FTP server, you can automate the creation of a CSV file, starting from the base dir where these files are stored, using the following js function:

```javascript
utils.createCsvFromFTPFolders(
    dirId,
    ftpProtocol, ftpHost, ftpPort, useSSL, ftpUsername, ftpPassword, ftpBaseDir,
    csvFileName, csvSeparator, fileNameAttributes, fileNameSeparator, folderAttributes
);
...
* dirId - base dir where saving the CSV file
* ftpXXX - settings needed to correctly connect to an FTP server and switch to a specific remote folder
* csvFileName - CSV file name to create
* csvSeparator - character to use to separate fields (e.g. "," or ";")
* fileNameAttributes - the parts composing the file name, separated with each other by "fileNameSeparator"
* fileNameSeparator - the character to use for separing attributes in the file name (e.g. "\_")
...
utils.createCsvFromFTPFolders(
    ...
    "myftphostname",
    myportAsANumber,
    true, // for secured FTP, false otherwise
    "myusername",
    "mypassword",
    "/", // remote dir
    "cms.csv", // CSV file to create
